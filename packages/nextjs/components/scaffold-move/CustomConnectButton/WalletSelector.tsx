...
export function WalletSelector({ isModalOpen, setModalOpen, ...walletSortingOptions }: WalletSelectorProps) {
  ...
  return (
    <>
      <Button onClick={onWalletButtonClick} variant="outline" className="hidden md:flex">
        <Wallet className="mr-2 h-4 w-4" />  {connected ? buttonText : "Connect Wallet"}
      </Button>

      <AboutAptosConnect renderEducationScreen={renderEducationScreens}>
        {walletSelectorModalOpen && (
           <dialog open className="modal absolute text-background z-100 min-h-screen modal-open">
